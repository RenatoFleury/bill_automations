function mainCondominio() {
  ...
  let bill_data = Condominio.findBill(bill_type, sender, subject, period, folder_id);
  if (bill_data) {
    const file_id = Condominio.saveBill(folder_id, bill_data.file_name, bill_data.message);
    if (file_id) {
...
const Condominio = {
  ...
  findBill: function(bill_type, sender, subject , period) {
    ...
    if (threads.length > 0) {
      ...
      let value = Condominio.get_value(plainBody);
      let dead_line = Condominio.get_dead_line(plainBody); // DD/MM/YYYY
      let bar_code = Condominio.get_bar_code(plainBody);
  ...
  saveBill: function(folder_id, file_name, message) {
    if (!is_file_in_folder(file_name, folder_id)) {
      let file_link = Condominio.get_file_link(message.getBody());
